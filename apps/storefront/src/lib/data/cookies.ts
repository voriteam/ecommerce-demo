...
/**
 * How long a cached catalog response may go stale.
 *
 * The tags below only exist for visitors carrying a cache-id cookie, and
 * nothing outside the storefront can revalidate them - so a `force-cache`
 * fetch made before `pnpm seed:catalog` would otherwise serve the old shelves
 * forever. A minute keeps the caching worth having while making a re-seed show
 * up on its own.
 */
...
export const getCacheOptions = async (
  tag: string
): Promise<{ revalidate: number; tags?: string[] } | Record<string, never>> => {
  ...
  if (!cacheTag) {
    return { revalidate: CATALOG_REVALIDATE_SECONDS }
  }

  return { revalidate: CATALOG_REVALIDATE_SECONDS, tags: [`${cacheTag}`] }
}
