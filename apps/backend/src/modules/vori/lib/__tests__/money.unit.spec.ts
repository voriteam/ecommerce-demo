...
describe("money", () => {
  ...
  it("survives the sums a float would not", () => {
    expect(centsToDecimal(sumCents([decimalToCents("0.10")!, decimalToCents("0.20")!]))).toBe(
      "0.30",
    )
...
describe("rounding computed money", () => {
  it("rounds a computed tax rather than truncating it", () => {
    // Medusa keeps tax unrounded; a register charges cents. Truncating would
    // understate every taxed line by up to a cent.
    expect(decimalToCentsRounded("1.030425")).toBe(103)
    expect(decimalToCentsRounded("1.0375")).toBe(104)
    expect(decimalToCentsRounded("1.035")).toBe(104)
    expect(decimalToCentsRounded("1.034999")).toBe(103)
    expect(decimalToCentsRounded("12.49")).toBe(1249)
    expect(decimalToCentsRounded("-1.035")).toBe(-104)
  })

  it("still refuses anything that is not a number", () => {
    for (const value of [null, undefined, "", "free"]) {
      expect(decimalToCentsRounded(value)).toBeNull()
    }
  })

  it("differs from the price parser, which must never round a shelf price up", () => {
    expect(decimalToCents("4.999")).toBe(499)
    expect(decimalToCentsRounded("4.999")).toBe(500)
  })
})
