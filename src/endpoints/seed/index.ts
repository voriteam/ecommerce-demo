...
// Narrowed to the nav globals this seed actually resets. Typing it as the
// full GlobalSlug union makes `updateGlobal` accept only fields common to
// every global, which no longer includes navItems now that voriSync exists —
// and voriSync must not be cleared here anyway: wiping the watermark would
// send the next inventory poll back over the entire catalog.
